...
object ExternalApps {
    fun openAmap(context: Context, item: ItineraryItem): Boolean {
        val encodedName = URLEncoder.encode(item.title, "UTF-8")
        val encodedAddress = URLEncoder.encode(item.address, "UTF-8")
    ...
    fun openAmap(context: Context, entry: StoryEntry): Boolean = launch(
        context,
        Intent(Intent.ACTION_VIEW, Uri.parse("androidamap://keywordNavi?sourceApplication=旅迹&keyword=${URLEncoder.encode("${entry.title} ${entry.address}", "UTF-8")}&style=2"))
